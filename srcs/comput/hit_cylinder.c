...
/*   Updated: 2021/04/19 23:36:22 by eriling          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void get_bigx_bigy(t_vect *big_x, t_vect *big_y, t_obj *cy)
{
	...
	*big_y = cross_prod(vect_orien_cy, *big_x);
}
...
double	get_zp(double t, t_data *img, t_obj *cy)
{
	...
	vect_orien_cy = normalize(vect_orien_cylinder(cy));
	big_z = vect_orien_cylinder(cy);
	zp = t * dot(img->dir, big_z)
		+ dot(vect_sous(img->origin, vect_orien_cylinder(cy)), big_z);
	if (fabs(zp) <= cy->u.cylinder.high / 2)
		return (zp);
...
int	is_det_zero_inter_near(t_math *math, t_data *img, t_obj *cy)
{
	...
	if (math->t1 > 0 && math->t1 < img->t 
		&& get_zp(math->t1, img, cy) <= cy->u.cylinder.high / 2)
	{
		img->t = math->t1;
		return (1);
	}
	...
	if (math->det > 0)
	{
		...
		math->t2 = (-math->b - sqrt(math->det)) / (2 * math->a);
			...
			if (math->t2 > 0 && math->t2 < img->t 
				&& get_zp(math->t2, img, cy) <= cy->u.cylinder.high / 2)
				img->t = math->t2;
			return (1);
	}
	else if (math->det == 0)
